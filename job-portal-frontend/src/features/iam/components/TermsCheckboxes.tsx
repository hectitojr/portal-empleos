...
export default function TermsCheckboxes({
  acceptTerms, acceptDataPolicy, onChange, errors,
}: {
  acceptTerms: boolean;
    ...
    <>
      <label htmlFor={termsId} className="flex items-start">
        <input
          ...
          className={`w-4 h-4 text-blue-600 rounded border-slate-300 focus-visible:ring-blue-500 mt-1 ${errors.acceptTerms ? 'border-red-300' : ''}`}
        />
      ...
      {errors.acceptTerms && (
        <p className="mt-1 text-sm text-red-600 flex items-center gap-1" role="alert" aria-live="assertive">
      ...
      <label htmlFor={dataId} className="flex items-start">
        <input
          ...
          className={`w-4 h-4 text-blue-600 rounded border-slate-300 focus-visible:ring-blue-500 mt-1 ${errors.acceptDataPolicy ? 'border-red-300' : ''}`}
        />
        <span className="ml-2 text-sm text-slate-600">
